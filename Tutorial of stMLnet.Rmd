...
load("~/cell_cell_interaction/test_R/unprocessed_ex_inputs.rda")
load("~/cell_cell_interaction/test_R/ex_databases.rda")

de_count <- ex_raw_inputs$Mat
de_cell_type <- ex_raw_inputs$annoMat
de_coords <- ex_raw_inputs$locaMat

ex_inputs <- Select_Lig_Rec_TGs(ExprMat = de_count, AnnoMat = de_cell_type, 
                                        LocaMat = de_coords, Databases = ex_databases, 
                                        python_path = "/usr/bin/python3", min.pct = 0.05, 
                                        expr.ct = 0.1, pct.ct = 0.05)

```
If you download demo database and demo processed inputs from [here](https://www.zenodo.org/record/8024150), the demo processed inputs contains the `exprMat`,`annoMat`, `locaMat`, `ligs_of_inter`, `recs_of_inter`, and `tgs_of_inter`. You can start run stMLnet.
...
The `ligs_of_inter`, `recs_of_inter`, and `tgs_of_inter` can be automatically computed based on the aforementioned three input matrixes.
