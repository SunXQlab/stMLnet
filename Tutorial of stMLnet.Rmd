...
If you download demo database and demo data (unprecessed_ex_inputs.rda) from [here](https://www.zenodo.org/record/8024150), the demo data `unprocessed_ex_inputs` contains the `exprMat`,`annoMat`, and `locaMat`, you need to calculate the `ligs_of_inter`, `recs_of_inter`, and `tgs_of_inter` according to the following code before running stMLnet.
...
The processed data (`ex_inputs.rda`) containing the `exprMat`,`annoMat`, `locaMat`, `ligs_of_inter`, `recs_of_inter`, and `tgs_of_inter` can be downloaded from [here](https://www.zenodo.org/record/8024150), which can be used directly as the input of stMLnet analysis. 
```
