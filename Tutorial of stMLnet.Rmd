...
If you download demo database and demo unprecessed inputs from [here], the demo unprocessed `unprocessed_ex_inputs` contains the `exprMat`,`annoMat`, and `locaMat`, you need to calculate the `ligs_of_inter`, `recs_of_inter`, and `tgs_of_inter` according to the following code before running stMLnet.
...
ex_inputs <- CalculateCandidateGeneList(ExprMat = de_count, AnnoMat = de_cell_type, 
